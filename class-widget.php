	...
	public function post_thumbnail_html( $post_id, $post_thumbnail_id, $size, $attr ) {
		...
		foreach ( get_intermediate_image_sizes() as $thumb ) {
			if ( in_array( $thumb, array('thumbnail') ) ) {
				$thumbSize[0]  = get_option( "{$thumb}_size_w" );
				$thumbSize[1] = get_option( "{$thumb}_size_h" );
			}
		}
		if ( (bool) get_option( "{$thumb}_crop" ) ) {
			$thumbSize[0] = $size[0] <= $thumbSize[0] ? ( $thumbSize[0] + 1 ) : $size[0];
			$thumbSize[1] = $size[1] <= $thumbSize[1] ? ( $thumbSize[1] + 1 ) : $size[1];
		}
